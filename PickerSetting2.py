...
class PickerList(simpy.FilterStore):
    ...
    def __init__(self, env: simpy.Environment, num_of_pickers):
        ...
        self.items = [Picker(idx = i+1, area = math.floor(i/pickerlimit)+1,
                             aisle = int(random.randint(0,1)*(aislelimit-1)+math.floor(i/pickerlimit)*aislelimit+1), location=1) for i in range(num_of_pickers)]

    def get_picker(self, area, spot:list) -> [FilterStoreGet,list,list]:
        ...
        if (len(list1)>=1):
            ...
            if (distance1 < distance2):
                ...
                return super().get(lambda picker: picker.area == area), spot[0], spot[1]
            else:
                self.items.sort(key=lambda picker: picker.picker_travel_distance(spot[1]))
                return super().get(lambda picker: picker.area == area), spot[1], spot[0]
        else:
            return super().get(lambda picker: picker.area == area), spot[1], spot[0]
